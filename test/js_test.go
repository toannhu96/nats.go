...
func TestJetStreamPullOptions(t *testing.T) {
	withJSCluster(t, "POPTS", 3, testJetStreamPullOptions)
}

func testJetStreamPullOptions(t *testing.T, srvs ...*jsServer) {
	srv := srvs[0]
	...
	t.Run("batch size", func(t *testing.T) {
		sub, err := js.SubscribeSync(subject, nats.Durable("pd1"), nats.Pull(1))
		if err != nil {
			t.Fatal(err)
		}
		defer sub.Unsubscribe()
	...
	Loop:
		...
		for i := 0; i < 10; i++ {
			resp, err = sub.NextMsg(50 * time.Millisecond)
	...
	t.Run("no wait", func(t *testing.T) {
		sub, err := js.SubscribeSync(subject, nats.Durable("pd2"), nats.Pull(1))
		if err != nil {
			t.Fatal(err)
		}
		defer sub.Unsubscribe()
		...
		// Extra pull to buffer more messages
		//
		err = sub.Poll(nats.BatchSize(10), nats.PullNoWait())
		if err != nil {
			t.Fatal(err)
		}
		nc.Flush()
	...
	Loop:
		...
		for i := 0; i < totalPolls; i++ {
			sub.Poll(nats.BatchSize(1), nats.PullNoWait())
		}
		nc.FlushTimeout(1 * time.Second)
	...
	t.Run("pull expires in", func(t *testing.T) {
		sub, err := js.SubscribeSync(subject, nats.Durable("pd3"), nats.Pull(1))
		if err != nil {
			t.Fatal(err)
		}
		defer sub.Unsubscribe()
		...
		// 1 from Initial Pull + 9 extra from the batch request.
		//
		err = sub.Poll(nats.BatchSize(9), nats.PullExpiresIn(1*time.Second))
		if err != nil {
			t.Fatal(err)
		}
		nc.Flush()
	...
	Loop:
		...
		// Poll more than the default max of waiting/inflight pull requests.
		sub.Poll(nats.BatchSize(1), nats.PullExpiresIn(10*time.Millisecond))
		time.Sleep(20 * time.Millisecond)
		nc.FlushTimeout(1 * time.Second)

		// We will get only 408 timeout errors.
		checkForPending(t, sub, 1, 1*time.Second)
