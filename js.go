...
// PullExpiresIn defines the deadline for which a pull request should expire.
...
func (sub *Subscription) Poll(opts ...PullOpt) error {
	var o pullOpts
	for _, opt := range opts {
	...
	nr := &nextRequest{Batch: batch}
	if o.expires > 0 {
		expire := time.Now().Add(o.expires)
		nr.Expires = &expire
	}
	nr.NoWait = o.noWait
